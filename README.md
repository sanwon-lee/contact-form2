...
- githubへのコミットをもう少し頻繁に行い、デバッグをするときなどにコミットを積極的に活用すること
- ログイン済みであるユーザーに限りお問い合わせフォームを記入できるようにしてあるため、（将来ゲストユーザもお問い合わせフォームを送信できるようにしてみるけど、それは後の話）ログイン済みであるならば名前とメールアドレスの入力を省略できるようにする。それでもお問い合わせを保存するデータベースにはログインしている場合はユーザーIDを保存させる。
- Pestによるテストコードを書いてみること。githubへコミットする際にはテストをしてからパスしていることを確認した上で行うのが基本。
